<%@ Page Language="vb" AutoEventWireup="false" MasterPageFile="~/Pages/Pigeon.Master" CodeBehind="Receivables.aspx.vb" Inherits="Pigeon.Receivables" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="MainContent" runat="server">
        ...
        <script id="resultsTemplate" type="text/x-jquery-tmpl">
           <tr>
                   ...
                   <td> <input type="text" class="invoice-no" style="width:75px" value="${theInvoiceNo}"/> </></td>
                   <td>${thePayer} </td>
                   <td>${theInvoiceType} </td>
                   <td>${theContractNo} </td>
                   <td>${theAuthNo} </td> 
                   <td class="amount">${accounting.formatMoney(theAmount,"", 2, "", ".")}</td>
                   <td> <input type="text" class="amount-paid" style="width:60px" value="${accounting.formatMoney(theAmountPaid,"", 2, "", ".")}"/> </><img class='warning' style="display:none"  src="../images/Warning.png" width="12px" height="12px" /> </td> 
    ...
    <script type="text/javascript">
        ...
        $('document').ready(function () {
            ...
            //get vendors
            var urlMethod = "../AccountingWebService.asmx/GetReceivableCompanies";
            ...
            $('#btnGo').click(function () {
                ...
                var urlMethod = "../AccountingWebService.asmx/GetReceivable";
                var json = { 'companyid': $('#selCompany').val(), 'client': user.Client };
                var jsonData = JSON.stringify(json);
                SendAjax(urlMethod, jsonData, function (msg) {
                    response = jQuery.parseJSON(msg.d);
                    ...
                    $('.amount-paid').change(function () {
                        ...
                        var editamount = $(editrow).find('.amount')[0].innerHTML;
                        if ($(this).val() - editamount != 0) {
                            $(editrow).find('.warning')[0].style.display = 'inline';
                        } else {
                            $(editrow).find('.warning')[0].style.display = 'none';
                        }
                    });
