...
<ToolboxItem(False)>
Public Class ManualTransmissionWebService
        ...
        If year <> "null" And make <> "null" And model <> "null" Then
            ...
            Using conn As New SqlConnection(ConnectionStrings.CKConnectionString)
                Dim sqlComm As New SqlCommand(String.Format("SELECT DISTINCT
                         trans.Part as partno, cost, case when core is null then 0 else core end as core, note as specifics
FROM            ACES.dbo.Vehicle AS aces_vehicle LEFT OUTER JOIN
                         PartsManager.dbo.tblZumbrotaTransmissionCatalog AS trans ON aces_vehicle.BaseVehicleID = trans.BaseVehicleID RIGHT OUTER JOIN
                         VehicleData.dbo.LKP_VEH_AAIA AS dataone_aaia_cross ON aces_vehicle.VehicleID = dataone_aaia_cross.AAIA_VehicleID RIGHT OUTER JOIN
  ...
  INNER JOIN PartsManager.dbo.tblZumbrotaTransmissionPricing pricing on pricing.part=trans.part
                ...
                Using r As SqlDataReader = sqlComm.ExecuteReader()
                    While r.Read()
                        ...
                        p1.Description = r("Specifics")
                        p1.PartNumber = r("PartNo")

                        'local stock
                        If client = "CK" Or String.IsNullOrEmpty(clientmanualtransmissionstock) Then
                            p1.LocalStock = 0
                        Else
                            Using conn2 As New SqlConnection(ConnectionStrings.GetSpecificConnectionString(IIf(client = "AutoNation", GetDistributor(name), clientmanualtransmissionstock)))
                                Dim sqlComm8 As New SqlCommand(String.Format("SELECT count(Part) from tblStock WHERE part = '{0}' AND type = {1} AND Arrive IS NOT NULL AND ReturnType IS NULL and ckorderid is null", p1.PartNumber, Convert.ToInt32(PartTypes.ManualTransmission)), conn2)
                                conn2.Open()
    ...
    Public Function GetDataByVin(ByVal vin As String, ByVal name As String, ByVal client As String)
        ...
        Using conn As New SqlConnection(ConnectionStrings.CKConnectionString)
            Dim sqlComm As New SqlCommand(String.Format("SELECT DISTINCT
                         trans.Part as partno, cost, case when core is null then 0 else core end as core, note as specifics,dataone_vin.year,
                  dataone_vin.make,
                  dataone_vin.model
FROM            ACES.dbo.Vehicle AS aces_vehicle LEFT OUTER JOIN
                         PartsManager.dbo.tblZumbrotaTransmissionCatalog AS trans ON aces_vehicle.BaseVehicleID = trans.BaseVehicleID RIGHT OUTER JOIN
  ...
  INNER JOIN PartsManager.dbo.tblZumbrotaTransmissionPricing pricing on pricing.part=trans.part
        ...
        Return js.Serialize(list2)
    End Function
    Public Shared Function GetManualTransmissionPrice(ByVal partno As String, ByVal name As String, ByVal client As String)


        Dim decCKCost, decCKSell As Decimal

        Dim strPartType = "Manual Transmission"


        GetDefaults(client)

        'get initial full ck cost
        decCKCost = GetCKCatalogCost(partno, 10, 91)
        ...
        override = VendorOverride(partno, PartTypes.ManualTransmission, 91)
        If (client = "CK" Or clienttype <> "PremierWholesaler") And client <> "DickMyers" Then
            If override.Override = True Then
                ...
                decCKCost = GetCKCatalogCost(p1.partno, PartTypes.ManualTransmission, p1.vendor)
            End If
        End If

        p1.warranties = GetWarranty(PartTypes.ManualTransmission, name, client)
        'get our all important base Tier(2) to mark on top of this
        decCKSell = GetCKSell(decCKCost, PartTypes.ManualTransmission)
        'see if pretty, if so set sell to this new price
        Dim decPrettyPrice As Integer
        decPrettyPrice = GetPrettyPrice(partno, PartTypes.ManualTransmission, p1.vendor)
        If (client = "CK" Or clienttype <> "PremierWholesaler") And client <> "DickMyers" And decPrettyPrice > 0 Then
            p1.tiers = GetTiers(PartTypes.ManualTransmission, name, client, decPrettyPrice, "pretty")
        Else
            p1.tiers = GetTiers(PartTypes.ManualTransmission, name, client, decCKSell, "sell")
        End If

        'see if price override, if so set sell to this new price
        Dim decOverridePrice As Integer
        decOverridePrice = GetOverridePrice(partno, 10, p1.vendor, name, client)
        If decOverridePrice > 0 And client <> "CK" Then
            p1.tiers = GetTiers(10, name, client, decOverridePrice, "pretty")
        ...
        If client <> "CK" Then
            For Each tier As VisibleTier In p1.tiers
                If tier.TierID <> 1 And tier.TierID <> 2 Then
                    decPrettyPrice = GetPigeonPrettyPrice(partno, PartTypes.ManualTransmission, p1.vendor, tier.TierID, client)
        ...
        clientmanualtransmissionstock = IIf(client = "AutoNation", GetDistributor(name), clientmanualtransmissionstock).ToString
        If String.IsNullOrEmpty(clientmanualtransmissionstock) Then
            p1.localstock = 0
        Else
            p1.localstock = GetLocalStock(partno, 10, clientmanualtransmissionstock)
        End If
        ...
        If p1.localstock > 0 Then 'locally available, get delivery map data
            p1.maps = GetMapData(partno, 10, clientmanualtransmissionstock, True, IIf(client = "AutoNation", False, True), IIf(client = "AutoNation", False, True), p1.vendor)
            If client = "AutoNation" Then
                'set local distributor as closet warehouse
                objClosestWarehouse = GetDistributorWarehouse(clientmanualtransmissionstock, name)
            End If
        Else 'check others
            If client = "AutoNation" And (GetCustomerState(GetCustomerNo(name, "AutoNation"), "AutoNation") = "CO" Or GetCustomerState(GetCustomerNo(name, "AutoNation"), "AutoNation") = "WA") Then
                ...
                p1.maps = GetMapData(partno, PartTypes.ManualTransmission, clientmanualtransmissionstock, False, True, True, p1.vendor, , True)
            Else
                p1.maps = GetMapData(partno, PartTypes.ManualTransmission, clientmanualtransmissionstock, False, True, True, p1.vendor)
            End If

            If client = "AutoNation" Then
                'back out any markup for the dealership tier
                Dim decNewPrice As Decimal = GetClientCost(partno, PartTypes.ManualTransmission, 91, "AutoNation")
                ...
                Else
                    ...
                    Dim objNonAutoNationMaps = GetMapData(partno, 10, clientmanualtransmissionstock, False, True, True, p1.vendor, , IIf(GetCustomerState(GetCustomerNo(name, "AutoNation"), "AutoNation") = "CO" Or GetCustomerState(GetCustomerNo(name, "AutoNation"), "AutoNation") = "WA", True, False), True)
    ...
    Public Function InsertNewQuote(ByVal customerNo As String, ByVal customerEmail As String, ByVal year As String, ByVal make As String, ByVal model As String, ByVal name As String, ByVal client As String, ByVal partNo As String, ByVal vin As String)
        Dim js As New JavaScriptSerializer
        Try
            Dim priceInfo As Pricing = GetManualTransmissionPrice(partNo, name, client)
            ...
            If vin <> "" Or vin <> String.Empty Then
                add1.quoteID = ProcessQuoteWithVIN(customerNo, customerEmail, name, year, make, model, Nothing, Nothing, Nothing, client, partNo, priceInfo.localstock, decSellPrice, priceInfo, PartTypes.ManualTransmission, Nothing, Nothing, vin)
            Else
                add1.quoteID = ProcessQuote(customerNo, customerEmail, name, year, make, model, Nothing, Nothing, Nothing, client, partNo, priceInfo.localstock, decSellPrice, priceInfo, PartTypes.ManualTransmission, Nothing, Nothing)
            End If
    ...
    Public Function GetPrice(ByVal year As String, ByVal make As String, ByVal model As String, ByVal customerNo As String, ByVal customerEmail As String, ByVal partno As String, ByVal name As String, ByVal client As String, ByVal customerClient As String, ByVal vin As String, ByVal tierID As String)
        ...
        Try
            Dim priceInfo As Pricing = IIf(customerClient <> client, GetManualTransmissionPrice(partno, GetUserNameByEmail(customerEmail, customerClient), customerClient), GetManualTransmissionPrice(partno, name, client))
            Try
                If GetUserRole(name, client) = "Admin" Then
                    If customerClient <> client Then
                        adminTiers = getVisibleTiers(GetUserEmail(name, client), client, GetManualTransmissionPrice(partno, name, client).tiers, tierID)
                        ...
                        Try
                            pigeonTiers = SearchManualTransmissionByPartNumber(partno, customerClient, pigeonAdmin)(0).tiers
                    ...
                    If client <> "CK" Then
                        ...
                        Else
                            Try
                                pigeonTiers = SearchManualTransmissionByPartNumber(partno, client, pigeonAdmin)(0).tiers
    ...
    Public Function GetQuotes(ByVal client As String)
        ...
        Using conn As New SqlConnection(ConnectionStrings.GetSpecificConnectionString(client))
            ...
            where tblRemanQuotes.PartTypeID=" & Convert.ToInt32(PartTypes.ManualTransmission) & " order by QuoteID desc", conn)
