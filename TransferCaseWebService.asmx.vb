...
<ToolboxItem(False)>
Public Class TransferCaseWebService
    ...
    <WebMethod()>
    Public Function GetData(ByVal source As String, ByVal year As String, ByVal make As String, ByVal model As String, ByVal name As String, ByVal client As String)
        ...
        If year <> "null" And make <> "null" And model <> "null" Then
            Using conn As New SqlConnection(ConnectionStrings.CKConnectionString)
                Dim sqlComm As New SqlCommand(String.Format("SELECT DISTINCT
                  pricing.part as partno, cost, case when core is null then 0 else core end as core, note as description
                FROM ACES.dbo.Vehicle AS aces_vehicle LEFT OUTER JOIN
                  PartsManager.dbo.tblZumbrotaTransferCaseCatalog AS tcase ON aces_vehicle.BaseVehicleID = tcase.BaseVehicleID
                  RIGHT OUTER JOIN
                  ...
                  INNER JOIN PartsManager.dbo.tblZumbrotaTransferCasePricing as pricing on pricing.part=tcase.part
                WHERE (dataone_vin.year='{0}' and dataone_vin.make='{1}' and dataone_vin.model='{2}')
                ORDER BY partno", year, make, model), conn)
                conn.Open()
                Using r As SqlDataReader = sqlComm.ExecuteReader()
                    While r.Read()
                        ...
                        'local stock
                        If client = "CK" Or String.IsNullOrEmpty(clienttransfercasestock) Then
                            p1.LocalStock = 0
                        Else
                            Using conn2 As New SqlConnection(ConnectionStrings.GetSpecificConnectionString(IIf(client = "AutoNation", GetDistributor(name), clienttransfercasestock)))
                                Dim sqlComm8 As New SqlCommand("Select count(Part) From tblStock WHERE part = '" & p1.PartNumber & "' AND type = 4 AND Arrive IS NOT NULL AND ReturnType IS NULL and ckorderid is null", conn2)
                                conn2.Open()
                        ...
                        '    strsqlins = "insert into tbltransfercasequotes(quotedate, username, customerno, year, make, model, partno, localstock) values ('" & Now() & "','" & name & "','" & GetCustomerNo(name, client) & "','" & year & "','" & make & "','" & model & "','" & p1.PartNumber & "','" & p1.LocalStock & "')"
    ...
    Public Function GetDataByVin(ByVal vin As String, ByVal name As String, ByVal client As String)
        ...
        Using conn As New SqlConnection(ConnectionStrings.CKConnectionString)
            Dim sqlComm As New SqlCommand(String.Format("SELECT DISTINCT
                  pricing.part as partno, cost, case when core is null then 0 else core end as core, note as description,dataone_vin.year,
                  dataone_vin.make,
                  dataone_vin.model
                FROM ACES.dbo.Vehicle AS aces_vehicle LEFT OUTER JOIN
                  PartsManager.dbo.tblZumbrotaTransferCaseCatalog AS tcase ON aces_vehicle.BaseVehicleID = tcase.BaseVehicleID
            ...
            Using r As SqlDataReader = sqlComm.ExecuteReader()
                While r.Read()
                    ...
                    Else
                        Using conn2 As New SqlConnection(ConnectionStrings.GetSpecificConnectionString(IIf(client = "AutoNation", GetDistributor(name), clienttransfercasestock)))
                            Dim sqlComm8 As New SqlCommand(String.Format("Select count(Part) From tblStock WHERE part = '{0}' AND type = {1} AND Arrive IS NOT NULL AND ReturnType IS NULL and ckorderid is null", p1.PartNumber, Convert.ToInt32(PartTypes.TransferCase)), conn2)
                            conn2.Open()
                            p1.LocalStock = sqlComm8.ExecuteScalar
                        End Using
                    End If
                    ...
                    Dim strsqlins As String
                    Using conn2 As New SqlConnection(ConnectionStrings.GetSpecificConnectionString(client))
                        strsqlins = "insert into tbltransfercasequotes(quotedate, username, customerno, year, make, model, partno, localstock) values ('" & Now() & "','" & name & "','" & GetCustomerNo(name, client) & "','" & r("year") & "','" & r("make") & "','" & r("model") & "','" & p1.PartNumber & "','" & p1.LocalStock & "')"
                        Dim sqlCommins As New SqlCommand(strsqlins, conn2)
                        conn2.Open()
                        sqlCommins.ExecuteNonQuery()
                    End Using
        ...
        Return js.Serialize(list2)
    End Function
    Public Shared Function GetTransferCasePrice(ByVal partno As String, ByVal name As String, ByVal client As String)
        ...
        decCKCost = GetCKCatalogCost(partno, 4, 91)
        ...
        override = VendorOverride(partno, PartTypes.TransferCase, 91)
        If (client = "CK" Or clienttype <> "PremierWholesaler") And client <> "DickMyers" Then
            If override.Override = True Then
                ...
                decCKCost = GetCKCatalogCost(p1.partno, PartTypes.TransferCase, p1.vendor)
            End If
        End If

        p1.warranties = GetWarranty(PartTypes.TransferCase, name, client)
        'get our all important base Tier(2) to mark on top of this
        decCKSell = GetCKSell(decCKCost, PartTypes.TransferCase)
        'see if pretty, if so set sell to this new price
        Dim decPrettyPrice As Integer
        decPrettyPrice = GetPrettyPrice(partno, PartTypes.TransferCase, p1.vendor)
        If (client = "CK" Or clienttype <> "PremierWholesaler") And client <> "DickMyers" And decPrettyPrice > 0 Then
            p1.tiers = GetTiers(PartTypes.TransferCase, name, client, decPrettyPrice, "pretty")
        Else
            p1.tiers = GetTiers(PartTypes.TransferCase, name, client, decCKSell, "sell")
        End If

        'see if price override, if so set sell to this new price
        Dim decOverridePrice As Integer
        decOverridePrice = GetOverridePrice(partno, PartTypes.TransferCase, p1.vendor, name, client)
        If decOverridePrice > 0 And client <> "CK" Then
            p1.tiers = GetTiers(PartTypes.TransferCase, name, client, decOverridePrice, "pretty")
        ...
        If client <> "CK" Then
            For Each tier As VisibleTier In p1.tiers
                If tier.TierID <> 1 And tier.TierID <> 2 Then
                    decPrettyPrice = GetPigeonPrettyPrice(partno, PartTypes.TransferCase, p1.vendor, tier.TierID, client)
        ...
        clienttransfercasestock = IIf(client = "AutoNation", GetDistributor(name), clienttransfercasestock).ToString
        If String.IsNullOrEmpty(clienttransfercasestock) Then
            p1.localstock = 0
        Else
            p1.localstock = GetLocalStock(partno, PartTypes.TransferCase, clienttransfercasestock)
        End If
        ...
        If p1.localstock > 0 Then 'locally available, get delivery map data
            p1.maps = GetMapData(partno, PartTypes.TransferCase, clienttransfercasestock, True, IIf(client = "AutoNation", False, True), IIf(client = "AutoNation", False, True), p1.vendor)
            If client = "AutoNation" Then
                'set local distributor as closet warehouse
                objClosestWarehouse = GetDistributorWarehouse(clienttransfercasestock, name)
            End If
        Else 'check others
            If client = "AutoNation" And (GetCustomerState(GetCustomerNo(name, "AutoNation"), "AutoNation") = "CO" Or GetCustomerState(GetCustomerNo(name, "AutoNation"), "AutoNation") = "WA") Then
                ...
                p1.maps = GetMapData(partno, PartTypes.TransferCase, clienttransfercasestock, False, True, True, p1.vendor, , True)
            Else
                p1.maps = GetMapData(partno, PartTypes.TransferCase, clienttransfercasestock, False, True, True, p1.vendor)
            End If

            If client = "AutoNation" Then
                'back out any markup for the dealership tier
                Dim decNewPrice As Decimal = GetClientCost(partno, PartTypes.TransferCase, 91, "AutoNation")
                ...
                Else
                    ...
                    Dim objNonAutoNationMaps = GetMapData(partno, PartTypes.TransferCase, clienttransfercasestock, False, True, True, p1.vendor, , IIf(GetCustomerState(GetCustomerNo(name, "AutoNation"), "AutoNation") = "CO" Or GetCustomerState(GetCustomerNo(name, "AutoNation"), "AutoNation") = "WA", True, False), True)
    ...
    Public Function InsertNewQuote(ByVal customerNo As String, ByVal customerEmail As String, ByVal year As String, ByVal make As String, ByVal model As String, ByVal name As String, ByVal client As String, ByVal partNo As String, ByVal vin As String)
        ...
        Try
            ...
            Dim price As Pricing
            price = GetTransferCasePrice(partNo, name, client)

            Dim decSellPrice As Decimal
            For Each searchedtier As VisibleTier In price.tiers
            ...
            If vin <> "" Or vin <> String.Empty Then
                add1.quoteID = ProcessQuoteWithVIN(customerNo, customerEmail, name, year, make, model, Nothing, Nothing, Nothing, client, partNo, price.localstock, decSellPrice, price, PartTypes.TransferCase, Nothing, Nothing, vin)
            Else
                add1.quoteID = ProcessQuote(customerNo, customerEmail, name, year, make, model, Nothing, Nothing, Nothing, client, partNo, price.localstock, decSellPrice, price, PartTypes.TransferCase, Nothing, Nothing)
            End If
    ...
    Public Function GetPrice(ByVal year As String, ByVal make As String, ByVal model As String, ByVal customerNo As String, ByVal customerEmail As String, ByVal partno As String, ByVal name As String, ByVal client As String, ByVal customerClient As String, ByVal vin As String, ByVal tierID As String)
        ...
        If GetCustomerNo(name, client) = "1040" Or GetCustomerNo(name, client) = "1041" Then Exit Function
        Dim priceInfo As Pricing = IIf(customerClient <> client, GetTransferCasePrice(partno, GetUserNameByEmail(customerEmail, customerClient), customerClient), GetTransferCasePrice(partno, name, client))
        Try
            If GetUserRole(name, client) = "Admin" Then
                If customerClient <> client Then
                    adminTiers = getVisibleTiers(GetUserEmail(name, client), client, GetTransferCasePrice(partno, name, client).tiers, tierID)
                    ...
                    Try
                        pigeonTiers = SearchTransferByPartNumber(partno, customerClient, pigeonAdmin)(0).tiers
                ...
                If client <> "CK" Then
                    ...
                    Else
                        Try
                            pigeonTiers = SearchTransferByPartNumber(partno, client, pigeonAdmin)(0).tiers
        ...
        Else
            If vin <> "" Or vin <> String.Empty Then
                add1.quoteID = ProcessQuoteWithVIN(GetCustomerNo(name, client), GetUserEmail(name, client), name, year, make, model, Nothing, Nothing, Nothing, client, partno, priceInfo.localstock, decSellPrice, priceInfo, PartTypes.TransferCase, Nothing, Nothing, vin)
            Else
                add1.quoteID = ProcessQuote(GetCustomerNo(name, client), GetUserEmail(name, client), name, year, make, model, Nothing, Nothing, Nothing, client, partno, priceInfo.localstock, decSellPrice, priceInfo, PartTypes.TransferCase, Nothing, Nothing)
            End If
        End If
        response.Add(add1)
    ...
    Public Function GetQuotes(ByVal client As String)
        ...
        Using conn As New SqlConnection(ConnectionStrings.GetSpecificConnectionString(client))
            ...
            where tblRemanQuotes.PartTypeID=" & Convert.ToInt32(PartTypes.TransferCase) & " order by QuoteID desc", conn)
